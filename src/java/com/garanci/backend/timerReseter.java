 ...
 * @author zishan
 */
public class timerReseter extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int gmtTimeDiff = Integer.parseInt(request.getParameter("td"));

            DatabaseUtility utility = new DatabaseUtility();

            Connection connection = utility.getConnection();

            String insertQuery = "update scheduler_time_holder set hr=?,mn=?,gmt_time_diff=? where Id=1";

            try {

                PreparedStatement prepared = connection.prepareStatement(insertQuery);

                prepared.setInt(1, hr);

                prepared.setInt(2, mn);

                prepared.setInt(3, gmtTimeDiff);

                prepared.execute();

                /**
                 * change time to execute *
                 */
                out.print("done");

                TimeScheduler.getInstance().changeTimer(hr, mn, gmtTimeDiff);
